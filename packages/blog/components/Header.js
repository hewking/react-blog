      ...
      <Row type="flex" justify="center">
        ...
        <Col xs={0} sm={0} md={14} lg={8} xl={6}>
          <Menu mode="horizontal" className={styles.menu} onClick={handleClick}>
            <Menu.Item key="0" className={styles.ant_menu_item}>
              <HomeOutlined className={styles.header_menu_icon} />
              首页
            </Menu.Item>
            {typeArr &&
              typeArr.map((item, index) => {
                return (
                  <Menu.Item key={item.id} className={styles.ant_menu_item}>
                    {item.icon && typeIcon(item.icon, styles.header_menu_icon)}
                    {item.typeName}
                  </Menu.Item>
                );
              })}
